...
void *new_echo_client(void *event_loop, int send_count, const char *addr_string, int port)
{
    ...
    cctx->tcp_client = cio_new_tcp_connection(event_loop, cctx);
...
void free_echo_client(void *echo_client)
{
    ...
    cio_free_tcp_connection(cctx->tcp_client);
    free(cctx);
}
...
static void on_connect(void *ctx, int ecode)
{
    ...
    cio_tcp_connection_async_write(cctx->tcp_client, cctx->send_buf, strlen(cctx->send_buf), on_write);
}
...
void echo_client_start(void *echo_client, const char *addr, int port)
{
    ...
    cio_tcp_connection_async_connect(cctx->tcp_client, addr, port, on_connect);
}
