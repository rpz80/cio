...
/**
 * fd, address, addrlen - might be acquired with cio_resolv() from <cio/resolv.h>.
 * ctx - user-provided context. It will be passed to the async functions callbacks.
 */
void *cio_new_tcp_client(void *event_loop, void *ctx, int fd, const struct sockaddr *address,
    socklen_t addrlen);

/* Socket descriptor is removed from event_loop and closed within this call. */
void cio_free_tcp_client(void *tcp_client);

void cio_tcp_client_async_connect(void *tcp_client, void (*on_connect)(void *ctx, int ecode));
